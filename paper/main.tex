...
Density functional theory (DFT) calculations were performed to compute the electronic properties of FBn, dFBn, tFBn, and hFBn. All calculations were performed using the Gaussian 16 software package with the B3LYP exchange-correlation functional and the 6-31++G(d,p) basis set\cite{ref.16}. The "opt=tight" keyword was used to obtain the most stable structures for all individual diluent molecules. The highest occupied molecular orbital (HOMO) energy, lowest unoccupied molecular orbital (LUMO) energy, and the HOMO-LUMO energy gap in gas-phase were calculated for each molecule.
...
\begin{figure}[b]
    ...
    \begin{subfigure}{0.55\textwidth}
        \begin{overpic}[width=\linewidth]{../figures/feature_analysis/shap_summary.pdf}
            \put(-1,52){\textbf{(A)}}
        \end{overpic}
    \end{subfigure}
    \begin{subfigure}{0.44\textwidth}
        \begin{overpic}[width=\linewidth]{../figures/feature_analysis/loo_shap_uncertainty.pdf}
            \put(-1,65){\textbf{(B)}}
        \end{overpic}
    \end{subfigure}
    \caption{(A) SHAP values and (B) Leave-one-out SHAP uncertainty analysis of diluent features to ionic conductivity.}
    \label{Fig.6}
\end{figure}
...
To further assess the robustness of the SHAP-based feature interpretation, a leave-one-out (LOO) uncertainty analysis was performed (Figure~\ref{Fig.6}B). In each LOO subset, one diluent was removed, the Random Forest model was refitted using the remaining three diluents, and the mean absolute SHAP value of each molecular descriptor was recalculated. The LOO-SHAP analysis showed that molecular weight exhibited the largest mean absolute SHAP value, followed by viscosity coefficient, LUMO energy, and the HOMO/LUMO gap, suggesting that molecular size, transport-related physical properties, and frontier-orbital descriptors may jointly contribute to the observed conductivity variation. However, the confidence intervals were broad for most descriptors, reflecting the limited number of diluents included in the present dataset. Therefore, the SHAP ranking should not be interpreted as statistically definitive evidence of descriptor importance, but rather as preliminary, hypothesis-generating evidence to guide the selection of additional diluents for future experimental validation.

Spearman correlation coefficients and the corresponding p-values were also calculated to contextualize the feature-importance analysis. Because the feature-analysis dataset contains only four diluents, none of the correlations reached statistical significance at $p < 0.05$; for $n = 4$, even a perfect monotonic relationship gives a minimum two-sided p-value of 0.083. Accordingly, the correlation, RF feature-importance, and SHAP analyses are interpreted as hypothesis-generating rather than confirmatory evidence. In addition, because the HOMO-LUMO gap is mathematically derived from the HOMO and LUMO energies, the three DFT-derived descriptors are not statistically independent. Their individual feature-importance values should therefore be interpreted with caution and discussed collectively as frontier-orbital descriptors rather than as separate causal factors.
